...
as $$
  with
  -- All registered devices (auto-detects new devices added to rooms table).
  all_devices as (
    select id as device_id from public.rooms
  ),
  -- Per-device: the end of the last successfully archived report.
  -- NULL means this device has never been archived.
  last_archived as (
    select
      device_id,
  ...
  -- Per-device: the target archive window.
  -- For never-archived devices: last 7 days of available data.
  -- For previously archived devices: 7 days immediately after the last report.
  target_windows as (
    select
      d.device_id,
      coalesce(
  ...
  -- Count readings in the target window per device.
  window_readings as (
    select
      tw.device_id,
      tw.window_start,
  ...
  where
    -- Only archive if the window has closed (grace period passed).
    now() >= wr.window_end + make_interval(mins => grace_minutes)
    -- Require at least 1 reading to avoid empty reports.
    and wr.reading_count >= 1
    -- Deduplication: skip if this exact (device, start, end) was already archived.
    and not exists (
